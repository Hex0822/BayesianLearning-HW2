...
data = read.csv("https://github.com/mattiasvillani/BayesLearnCourse/raw/master/assignment/bugs.csv", 
                header = TRUE)
y = data$nBugs 
# response variable: the number of bugs, a vector with n = 91 observations
X = data[,-1]  
# 91 x 5 matrix with covariates
X = as.matrix(X) 
# X was initially a data frame, but we want it to be matrix
```
...
# A legal Sigma must be positive definite, 
# but hessian in fnscale=-1 is negative definite,
# So we fetch negative hessian.
...
par(mfrow=c(2,3))
# set a vector to store significant status
...
for (i in 1:ncol(sample)){
  ...
  #legend("topright",legend=c("95% CI","Zero Line"),
  #        col=c("red","green"),
  #        lty=c(2,1),
  #        bty="n")
...
Bonus: Here's the implementation of Highest Posterior Density Interval.
...
for (i in 1:ncol(sample)){
  ...
  hist(param_sample,
    main = paste("HDI in ", param_name),
    xlab = 'Parameter Value',
    col = "blue")
  
  hpdi_interval = hdi(param_sample, credMass = 0.95)
  abline(v = hpdi_interval, col = 'red', lty=2,lwd=2)
}
```